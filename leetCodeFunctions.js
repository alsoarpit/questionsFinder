...
async function top100LikedQuestions(){
    ...
    spinner.text = 'Creating... LeetCode Top 100 Liked Questions Pdf';
    spinner.start();
    ...
    let ansObj = await gPage.evaluate(()=>{
      ...
      await  folderCheck();
      spinner.succeed(chalk.bold.yellow(`Congratulations leetCode Top 100 Liked Questions PDF has been Created`));
...
async function top100CuratedAlgo(){
    try{
    spinner.spinner = "arc";
	spinner.color = 'yellow';
    spinner.text = 'Creating... LeetCode Top 100 Curated Algo Questions Pdf';
    spinner.start();
    var browser = await puppeteer.launch({
        headless:false,
        defaultViewport: null,
    ...
    let pagesArr = await browser.pages();
    var gPage=pagesArr[0];
    await gPage.goto("https://leetcode.com/problemset/all/?listId=552y65ke");
    await gPage.waitForTimeout(1100);
    await gPage.click('#headlessui-listbox-button-13');
    await gPage.waitForTimeout(500);
    await gPage.evaluate(()=>{   
    ...
    let ansObj = await gPage.evaluate(()=>{
        ...
        var levelArr = ansObj.levelArr;
      await  folderCheck();
      spinner.succeed(chalk.bold.yellow(`Congratulations leetCode Top 100 Curated Algo Questions PDF has been Created`));
       console.log(center((chalk.bgRedBright.bold.black("\n Thank You For Using questionsFinder ")),122));
    function folderCheck(){
        let folderPath = "./questionsFinder_Downloads/leetCodeTop100CuratedAgloQuestions_Questions"
        if (fs.existsSync(folderPath)) {
            pdfCreate(folderPath,questionArr,linkArr,acceptanceArr,levelArr);
        }else {
            fs.mkdirSync(folderPath,{ recursive: true });
    ...
    function pdfCreate(folderPath,questionArr,linkArr,acceptanceArr,levelArr){
        const doc = new PDFDocument;
        doc.pipe(fs.createWriteStream(folderPath+"/leetCodeTop100CuratedAgloQuestions_Questions.pdf"))     
            doc.font('./fonts/CascadiaCode-Bold.otf')
                .fontSize(22)
                .text(`LeetCode Top 100 Curated Algo Questions`)
            doc.moveDown();
            doc.font('./fonts/CascadiaCode.ttf')
                .fontSize(18)
                for(let i = 0; i <questionArr.length;i++){
...
async function top100InterviewQuestions(){
    try{
    spinner.spinner = "arc";
	spinner.color = 'yellow';
    spinner.text = 'Creating... LeetCode Top 100 Interview Questions Pdf';
    spinner.start();
    var browser = await puppeteer.launch({
        headless:false,
        defaultViewport: null,
    ...
    let pagesArr = await browser.pages();
    var gPage=pagesArr[0];
    await gPage.goto("https://leetcode.com/problemset/all/?listId=wpwgkgt");
    await gPage.waitForTimeout(1100);
    await gPage.click('#headlessui-listbox-button-13');
    await gPage.waitForTimeout(500);
    await gPage.evaluate(()=>{   
    ...
    let ansObj = await gPage.evaluate(()=>{
        ...
        var levelArr = ansObj.levelArr;
      await  folderCheck();
      spinner.succeed(chalk.bold.yellow(`Congratulations leetCode Top 100 Interview Questions PDF has been Created`));
       console.log(center((chalk.bgRedBright.bold.black("\n Thank You For Using questionsFinder ")),122));
    function folderCheck(){
        let folderPath = "./questionsFinder_Downloads/leetCodeTop100InterviewQuestions_Questions"
        if (fs.existsSync(folderPath)) {
            pdfCreate(folderPath,questionArr,linkArr,acceptanceArr,levelArr);
        }else {
            fs.mkdirSync(folderPath,{ recursive: true });
    ...
    function pdfCreate(folderPath,questionArr,linkArr,acceptanceArr,levelArr){
        const doc = new PDFDocument;
        doc.pipe(fs.createWriteStream(folderPath+"/leetCodeTop100InterviewQuestions_Questions.pdf"))     
            doc.font('./fonts/CascadiaCode-Bold.otf')
                .fontSize(22)
                .text(`LeetCode Top 100 Curated Algo Questions`)
            doc.moveDown();
...
module.exports ={top100LikedQuestions,top100CuratedAlgo,top100InterviewQuestions}
